...
times['load teacup'] = timeit.timeit("pysd.read_vensim('tests/old_tests/vensim/Teacup.mdl')",
                                     setup='import pysd', number=10)/10
...
times['run teacup baseline'] = timeit.timeit("model.run()",
                                    setup="import pysd; model = pysd.read_vensim('tests/old_tests/vensim/Teacup.mdl')",
                                    number=100)/100
...
times['monte carlo'] = \
    timeit.timeit("[model.run(initial_condition=(0, {'teacup_temperature':50*np.random.rand()})) for i in range(1000)]",
                  setup="import numpy as np; import pysd; model = pysd.read_vensim('tests/old_tests/vensim/Teacup.mdl')",
                  number=1)/1
